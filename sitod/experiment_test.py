...
class Experiment(Registrable):
    ...
    def _run_experiment(self, data_root_dir: Path, experiment_dir: Path) -> None:
    ...
    def run_experiment(self,
        ...
        # https://www.techiedelight.com/ko/write-to-file-python/
        (experiment_root_dir / OutputPaths.METADATA).write_text(json.dumps(metadata, indent=True))

        """ e.g.
        --data_root_dir: /data02/jeiyoon_park/dstc11-track2-intent-induction/dstc11
        --experiment_root_dir: /data02/jeiyoon_park/dstc11-track2-intent-induction/results
        """
...
Task 2
"""
# schema (intents with corresponding sample utterances) from a list of dialogues.
...
class OpenIntentInductionExperiment(Experiment):
    def __init__(self,
        ...
        :param dialogue_reader:
            reader for input dialogues
        :param dialogues_path:
            relative path to dialogues from dataset root
        :param run_id:
            unique identifier for this experiment
        :param metadata:
            dictionary containing metadata associated with experiment
        :param ignored_labels:
            reference labels to exclude from evaluation
        :param overwrite:
            whether to overwrite any pre-existing results
        :param test_utterances_path:
            relative path to test utterances file from dataset root
        :param open_intent_induction_model:
            model used for open intent induction
        :param test_dialogue_reader:
            dialogue reader used for test data
        :param classifier_evaluator:
            evaluator
            used for prediction-based evaluation of induced intents
        :param eval_seeds:
            random seeds used for prediction-based evaluation of induced intents
        """
        super().__init__(run_id, metadata)
        self._dialog_reader = dialogue_reader
        ...
        self._classsifier_evaluator = classifier_evaluator
    ...
    def _run_experiment(
        ...
        # induce schema
        # https://ponyozzang.tistory.com/438
        ...
        # https://docs.python.org/ko/3/library/numbers.html
