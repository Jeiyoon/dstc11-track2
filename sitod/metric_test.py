...
from typing import List, Dict, Tuple, Any, Counter as CounterT
...
def compute_optimal_alignment(predicted_labels: List[str], reference_labels: List[str]) -> Dict[str, str]:
    ...
    # https://jimmy-ai.tistory.com/185
    ...
    # https://hongl.tistory.com/159
    # nan error: https://gmnam.tistory.com/281
...
def count_cluster_label_overlap(
    ...
    """
    Return
        the label overlap counts between two clusterings.
    """
...
class ClusteringMetric(object):
    ...
    def metric_name(self) -> str:
        """
        Returns
            the name of the clustering metric for reporting.
        """
        raise NotImplementedError

    def compute_metric(self,
                       cluster_labels: List[str],
                       reference_labels: List[str]) -> float:
        """
...
class NMI(ClusteringMetric):
    ...
    Normalized Mutual Information (NMI):
        between predicted and reference clustering
    """
    def metric_name(self) -> str:
        return 'NMI'
...
class ClusteringPrecision(ClusteringMetric):
    ...
    Clustering precision,
        in which a many-to-one alignment is computed from *cluster* labels to *reference* labels
...
class ClusteringRecall(ClusteringMetric):
    """
    Clustering recall,
        in which a many-to-one alignment is computed from *reference* labels to *cluster* labels
        such that the number of correct aligned labels is maximized.
    """
...
class ExampleCoverage(ClusteringMetric):
    """
    Example coverage,
        defined as percent of examples
        whose reference intent has a corresponding predicted cluster
        after performing a many-to-one alignment
        from predicted clusters to reference clusters.
    """
...
def compute_many_to_one_alignment(cluster_labels: List[str], reference_labels: List[str]) -> Dict[str, str]:
    """
    Compute
        a many-to-one alignment from `cluster_labels` to `reference_labels` such that the total number
...
def compute_mean_and_stdev(
    ...
    for key, vals in metrics.items():
        ...
        # https://hooni-playground.com/1422/
...
def classification_metrics(predicted_labels: List[str], reference_labels: List[str]) -> Dict[str, Any]:
    ...
    # https://scikit-learn.org/stable/modules/generated/sklearn.utils.multiclass.unique_labels.html
    ...
    f1 = f1.tolist() + [micro_recall.tolist()]
...
def filter_labels(
    ...
    """
    Filter out
        any ignored instances and corresponding cluster labels
        given a list of ignored reference labels.
    """
